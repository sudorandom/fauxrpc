...
import (
	...
	context "context"
	errors "errors"
	v1 "github.com/sudorandom/fauxrpc/proto/gen/registry/v1"
	http "net/http"
	strings "strings"
)
...
var (
	registryServiceServiceDescriptor              = v1.File_registry_v1_registry_service_proto.Services().ByName("RegistryService")
	registryServiceAddDescriptorsMethodDescriptor = registryServiceServiceDescriptor.Methods().ByName("AddDescriptors")
	registryServiceResetMethodDescriptor          = registryServiceServiceDescriptor.Methods().ByName("Reset")
)
...
func NewRegistryServiceClient(httpClient connect.HTTPClient, baseURL string, opts ...connect.ClientOption) RegistryServiceClient {
	...
	return &registryServiceClient{
		addDescriptors: connect.NewClient[v1.AddDescriptorsRequest, v1.AddDescriptorsResponse](
			...
			connect.WithSchema(registryServiceAddDescriptorsMethodDescriptor),
			connect.WithClientOptions(opts...),
		),
		reset: connect.NewClient[v1.ResetRequest, v1.ResetResponse](
			httpClient,
			baseURL+RegistryServiceResetProcedure,
			connect.WithSchema(registryServiceResetMethodDescriptor),
			connect.WithClientOptions(opts...),
		),
	}
}
